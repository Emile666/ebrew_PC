...
%            Y(z^-1)       b1*z^-1 + b1*z^-2
% Gs(z^-1) = ------- = -----------------------
%            U(z^-1)     1 + p1*z^-1 + p1*z^-1
%
% This function is supplied with the input values u[k] and the 
% output values y[k] and calculates the values for b1, b2, a1 and a2.
%
...
% rho    - parameter rho
%-------------------------------------------------------------------
function [fi,theta,d,c,lambda,eta,ep,te,ksi,pp] = identdf_N1(fi,theta,d,c,lambda,eta,up,yk,rho)
